#!/usr/bin/env python3
"""orig: https://github.com/nxrates/repo2txt"""
...
""", file=sys.stderr)
    ...
    def output_stream() -> ContextManager[BinaryIO]:
        if not args.output:
    ...
    try:
        with output_stream() as out_file:
        ...
        print(f"\nDumped to {args.output or 'stdout'}. Bye!", file=sys.stderr)
    except OSError as e:
        print(f"Error writing to {args.output or 'stdout'}: {e}", file=sys.stderr)
        sys.exit(1)
